...
with open('../data/aggregated/medallion_agg_output/part-00000','rb') as f:
	lines = f.readlines()
f.close()
...
for line in lines:
	...
	medallion.append((parts[0], parts[1], datetime.datetime.strptime(parts[2], '%Y-%m-%d %H'), np.int64(cnt)))

trips = pd.DataFrame(medallion, columns=['borough', 'zone', 'time', 'count'])
...
timeidx = [start14 + datetime.timedelta(days=x) + datetime.timedelta(hours=y) for x in range(0, (end14-start14).days) for y in range(0,24)]
timeidx.extend(time15)
...
for z in trips.zone.unique():
	times_missing = list(set(timeidx)-set(trips[trips.zone==z].datetime.tolist()))
...
trips.to_csv('../data/aggregated/medallion_agg_final.csv', index=False)