...
# this script parses pickup time, lat, lon, fare, surcharge, and tip for each trip in 2015
...
tripsNoHeader = trips.filter(lambda l: "VendorID" not in l)
...
sel_fields = [fields[i] for i in [1, 5, 6, -7, -2, -4]]
schema = StructType(sel_fields)
trips_temp = tripsClean.map(lambda k: k.split(",")).map(lambda p: (datetime.strptime(p[1], "%Y-%m-%d %H:%M:%S"),float(p[5]), float(p[6]), float(p[-7]), float(p[-2]), float(p[-4])))
